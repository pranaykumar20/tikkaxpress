...
export default async function CancelledPage({ searchParams }: { searchParams: Promise<{ order_id?: string }> }) {
  ...
  return (
    <main className="grid min-h-screen place-items-center px-4 py-10">
      <section className="max-w-xl rounded-3xl bg-white p-8 text-center shadow-card">
        ...
        {params.order_id && <p className="mt-4 rounded-3xl bg-cream p-3 text-sm font-bold text-charcoal/60">Pending order reference: {params.order_id}</p>}
